...
public class JDBCInsert {
	...
	public static void main(String[] args) {
		...
		// INSERT INTO 구문을 이용해 DB에 데이터를 적재하도록 만들어 보겠습니다.
		
		// SELECT Q1 처럼 작성해주시되 (입력받기, DB연결, 쿼리문 실행등...)
		// SELECT 구문을 제외한 나머지 INSERT, DELETE, UPDATE 구문은
		// 결과 데이터가 없기 때문에 실행헀을때 에러가 나지 않는다면
		// 우선 실행된 것으로 볼 수 있고, 실행이 된 다음 워크벤치에서
		// 데이터가 들어갔는지 확인 해주시면 됩니다.
		
		// 쿼리문 실행시 SELECT 구문은 executeQuary(구문); 을 호출 하지만
		// SELECT이외 구문은 executeUpdate(구문); 을 호출 합니다.
