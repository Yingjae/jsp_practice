...
<%
	...
	// DB에 있던 자료를 꺼냈을때, 곧바로 java 데이터로 받아오는 작업을 수행한 다음
    ...
    // java.sql 관련 변수들도 앞으로 위에서 미리 선언해 둡니다.
    ResultSet rs = null; 
    Connection con = null;
    ...
    try{
	    ...
	    // 이 영역에서 바로 .close()를 쓰고도 데이터를 body 태그에
	    // .close() 이전에 DB에 있던 데이터를 모조리 자바로 옮겨야
	    con.close();
	    pstmt.close();
	    rs.close();
	}
    */
    ...
    UserDAO dao = UserDAO.getInstance(); // DB종류 지정까지 완료
...
<body>
	...
	Table 형태로 user_list1과 동일한 양식으로 화면에 출력해주세요. -->
	<table border="1">
		<thead>
			<tr>
				<th>유저이름</th>
