...
public class BoardDTO {
	...
	private static final int NAV_NUM = 5; // 페이지는 10개씩 링크 표시(하단에 깔린 버튼은 10개씩)
	...
	public BoardDTO(int boardCount, int currentPage) {
		// 총 글 갯수와, 현재 조회중인 페이지 정보를 입력받아 아래에 깔릴 버튼개수, 현재 깔려야 하는 번호대를 구합니다.
		...
		if(boardCount == 0) {
			...
			// <<게시글 개수를 이용해 천제 페이지 갯수 구하기>>
			// * 게시글 관련이므로 상수 BOARD_NUMdmf 사용합니다.
			if(boardCount % BOARD_NUM == 0) {
				// 전체 글 개수 % 10이 0으로 떨어짐 => 10의 배수 개수로 페이지 개수는 10을 나눈 결과값
			...
			if(currentPage % NAV_NUM == 0) {
				// 10의 배수 페이지는 자신이 속한 그룹보다 페이지 식별숫자가 1크게 나옴.
				navNum = (currentPage - 1) / NAV_NUM; // (50번 페이지의 경우 41~50그룹에 속하므로 실제로는 4가 나와야 하므로)
			} else {
				navNum = currentPage / NAV_NUM; // (51페이지 - 1 / 10 해도 5가 나옴, 50 - 1 / 10 하면 4가 나옴)
			}
			// navNum = (currentPage - 1) / NAV_NUM;
			
			// 식별번호 navNum이 구해졌으면 * NAV_NUM + 1을 해서 시작번호를 구합니다.
			...
			// endPage = (startPage + (NAV_NUM - 1) > tatalPages) ? tatalPages : startPage + (10 - 1);
		} 
